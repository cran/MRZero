...
setMethod("mr_mvgmm",
          ...
          function(object, nx, ny, cor.x=NULL, robust=TRUE, alpha=0.05, ...){
            ...
            if(robust == FALSE){
              ...
              if(sum(condF<0)>0) { condF = as.numeric(rep(NA, dim(Bx)[2])); message("Conditional F statistics did not converge to positive values - should the sample sizes be larger?\n") }
