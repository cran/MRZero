  ...
  function(object,
    ...
    if(distribution %in% c("normal", "t-dist")){
      ...
      if (length(v) > K){
        ...
        post_est = rep(NA, K)
        post_se = rep(NA, K)
        message("Specified value of lambda results in the same number of valid instruments as risk factors. Post-lasso method cannot be performed.")
      } else {
        post_est = rep(NA, K)
        post_se = rep(NA, K)
        message("Specified value of lambda results in fewer valid instruments than risk factors. Post-lasso method cannot be performed.")
      }
      ...
      message("Distribution must be one of : normal, t-dist. \nSee documentation for details. \n")
    }
  }
)