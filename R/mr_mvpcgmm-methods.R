...
setMethod("mr_mvpcgmm",
          ...
          function(object, nx, ny, cor.x=NULL, r = NULL, thres=0.999, robust=TRUE, alpha=0.05, ...){
            ...
            if(sum(is.na(object@correlation)) != 0){message("Genetic variant correlation (linkage disequilibrium) matrix is required but not provided. \n")}
            p <- nrow(bx); K <- ncol(bx)
            ...
            if(pc.check==1){message("More principal components needed to perform robust analysis. Consider a larger number for r or a larger threshold. \n")}
            ...
            if(robust == TRUE & pc.check == 0){
              ...
              if(default == 1){message("overdispersion parameter could not be estimated by uniroot. Default value of 0 selected, and unrobust estimates are given. \n")}
              ...
              message("The robust argument must be TRUE or FALSE. \nSee documentation for details. \n")
            }
  }
)