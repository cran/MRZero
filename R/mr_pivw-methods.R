...
setMethod("mr_pivw",
          ...
          function(object, lambda=1, over.dispersion=TRUE, delta=0, sel.pval=NULL, Boot.Fieller=TRUE, alpha=0.05){
            ...
            if(lambda<0){
              warning("\'lambda\' cannot be smaller than zero.","\n")
              return()
            }

            if(delta<0){
              warning("\'delta\' cannot be smaller than zero.","\n")
              return()
            }

            if(alpha<=0){
              alpha = 0.05
              message("\'alpha\' provided is less than or equal to zero. \'alpha\ is set to be 0.05.","\n")
            }

            if(delta>0){
              if(is.null(sel.pval)){
                delta=0
                message("\'sel.pval\' is not provided. \'delta\' is set to be zero and no IV selection conducted.","\n")
              }else if(length(sel.pval)!=p){
                delta=0
                message("The length of \'sel.pval\' doesn't match the number of snps in the MRInput object. \'delta\' is set to be zero and no IV selection conducted.","\n")
              }
            }
            ...
            if(delta==0){
              ...
              if(sum(sel)==0){
                warning("No snps is kept in the anaysis after IV selection. Please try a smaller \'delta\' value.","\n")
                return()
              }
